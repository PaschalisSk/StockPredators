# Local copy of finta with edits
import pandas as pd
...
class TA:
    ...
    def MACD(cls, ohlc, period_fast=12, period_slow=26, signal=9):
        ...
        EMA_fast = pd.Series(ohlc['close'].ewm(ignore_na=False, min_periods=period_slow, span=period_fast).mean(),
                             name='EMA_fast')
        EMA_slow = pd.Series(ohlc['close'].ewm(ignore_na=False, min_periods=period_slow, span=period_slow).mean(),
                             name='EMA_slow')
    ...
    def RSI(cls, ohlc, period=14):
        ...
        _gain = up.rolling(center=False, window=period, min_periods=period).mean()
        _loss = down.abs().rolling(center=False, window=period, min_periods=period).mean()
    ...
    @classmethod
    def STOCHD(cls, ohlc, period=14):
        """Stochastic oscillator %D
        STOCH%D is a 3 period simple moving average of %K.
        """

        return pd.Series(cls.STOCH(ohlc, period).rolling(center=False, window=3).mean(),
